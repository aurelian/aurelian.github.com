...
When I started to code on "medick":http://medick.locknet.ro, one of my first goals was to hide all the PHP annoyances, to hide them from me because I will be the main user of this framework and then to hide them from my users. At one point I knew that I will face some of those annoyances, where I will loose more that 15 minutes to solve.

So, on my "TODO":http://www.locknet.ro/article/erata-medick-021 list for release 0.2.2 I had:
...
Anyway, I have started to modify this class to match my "ISessionContainer":http://medick.locknet.ro/api/medick.action.controller/session/ISessionContainer.html interface, a piece of cake. Ended up with a nice class declaration:
...
To test somehow, I altered my "Session":http://medick.locknet.ro/api/medick.action.controller/session/Session.html class and made a call to that function in my "start":http://medick.locknet.ro/api/medick.action.controller/session/Session.html#start method, just before session_start:
